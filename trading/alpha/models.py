...
class MeanReversionAlpha:
    ...
    def generate(self, context: PriceSeriesContext) -> list[Signal]:
        ...
        return [Signal("mean_reversion", context.instrument, direction, strength, min(0.5 + strength / 2, 1.0),
                       self.horizon_seconds, metadata={"zscore": z})]
...
class MomentumAlpha:
    ...
    def generate(self, context: PriceSeriesContext) -> list[Signal]:
        ...
        return [Signal("momentum", context.instrument, 1.0 if ret > 0 else -1.0, strength,
                       min(0.55 + strength * 0.35, 0.95), self.horizon_seconds,
...
class BreakoutAlpha:
    ...
    def generate(self, context: PriceSeriesContext) -> list[Signal]:
        ...
        if last > upper:
            return [Signal("breakout", context.instrument, 1.0, 0.7, 0.7, self.horizon_seconds,
...
class TrendFollowingAlpha:
    ...
    def generate(self, context: PriceSeriesContext) -> list[Signal]:
        ...
        return [Signal("trend_following", context.instrument, 1.0 if separation > 0 else -1.0,
                       strength, min(0.6 + strength * 0.3, 0.9), self.horizon_seconds,
